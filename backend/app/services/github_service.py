...
class GitHubService:
    ...
    def get_repository_structure(self, repo_full_name: str, max_files: int = 400) -> Dict[str, Any]:
        ...
        API call instead of many. This matters a lot for large repos:
        without it, the model only sees the root directory and has to
        blindly guess-and-search its way deeper, which is exactly what
        caused solution generation to time out on larger codebases.

        Common noise directories (node_modules, .git, build output,
        vendored deps) are filtered out so the token budget goes to
        source files, not clutter.
        """
        _SKIP_DIR_PARTS = {
        ...
        try:
            ...
            files = []
            truncated_by_github = getattr(tree, "truncated", False)
            for item in tree.tree:
                if item.type != "blob":
                    continue
                path_parts = item.path.split("/")
                if any(part in _SKIP_DIR_PARTS for part in path_parts):
                    continue
                files.append({"path": item.path, "type": "file"})
            ...
            return {
                ...
                "files": files,
                "truncated": truncated_by_github or len(files) >= max_files,
            }
        except Exception as e:
            logger.warning("structure_fetch_failed", error=str(e))
            # Fall back to the shallow root listing rather than nothing --
            # partial orientation still beats none.
            try:
                repo = self.get_repository(repo_full_name)
                contents = repo.get_contents("")
                items = contents if isinstance(contents, list) else [contents]
                files = [{"path": i.path, "type": i.type} for i in items[:50]]
                return {"repository": repo_full_name, "language": repo.language, "files": files, "truncated": True}
            except Exception as fallback_error:
                return {"repository": repo_full_name, "error": str(fallback_error), "files": []}
    ...
    def discover_issues(
        ...
        """Search GitHub-wide for open issues worth working on, e.g.
        'good first issue' / 'help wanted', optionally filtered by
        language. Sorted so genuinely unclaimed, recently-active issues
        surface first (few comments = probably nobody's working on it,
        recently updated = the repo is still alive).
        """
        labels = labels or ["good first issue"]
        try:
            label_query = " ".join(f'label:"{l}"' for l in labels)
            query_parts = ["is:issue", "is:open", label_query]
            if language:
                query_parts.append(f"language:{language}")
            query = " ".join(p for p in query_parts if p)

            results = self.client.search_issues(query=query, sort="updated", order="desc")
            ...
            return {"success": True, "query": query, "issues": issues}
        except GithubException as e:
            logger.warning("discover_issues_failed", error=str(e))
            return {"success": False, "error": str(e), "issues": []}

    def search_repos(self, query: str, max_results: int = 10) -> Dict[str, Any]:
        """Search for repos by name, so a user can pick one without
        needing to know its exact owner/repo path upfront."""
        try:
            results = self.client.search_repositories(query=query, sort="stars", order="desc")
            repos = []
            for i, item in enumerate(results):
                if i >= max_results:
                    break
            ...
            logger.warning("search_repos_failed", error=str(e))
    ...
    def list_repo_issues(
        ...
        """List issues for one specific, already-chosen repo -- this is
        the 'browse a repo I picked' path, as opposed to discover_issues()
        which searches across all of GitHub."""
        try:
            repo = self.get_repository(repo_full_name)
            kwargs = {"state": state}
            if labels:
                kwargs["labels"] = labels
            issue_list = repo.get_issues(**kwargs)
            ...
            return {"success": True, "repo_full_name": repo_full_name, "issues": issues}
        except GithubException as e:
            logger.warning("list_repo_issues_failed", repo=repo_full_name, error=str(e))
            return {"success": False, "error": str(e), "issues": []}
