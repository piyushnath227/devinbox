r"""Coordinates the full issue-to-PR pipeline:
...
class AgentOrchestrator:
    ...
    def process_issue(
        ...
        try:
            ...
            self._log(
                ...
                f"using {len(tool_calls)} repo tool call(s), confidence {solution_confidence:.0%}",
                metadata=solution, tokens_used=sol_result.get("tokens_used"), latency_ms=sol_result.get("latency_ms"),
            )

            # Confidence gate: a low-confidence automated fix is more
            # costly to everyone (a wrong PR cluttering a maintainer's
            # repo) than a comment saying "I looked, but I'm not sure."
            from ..config import get_settings
            min_confidence = get_settings().MIN_SOLUTION_CONFIDENCE
            if solution_confidence < min_confidence:
                return self._handle_low_confidence(issue, solution, repo_full_name, issue_number, start_time)
    ...
    def _handle_low_confidence(self, issue, solution, repo_full_name, issue_number, start_time):
        """Solution generation succeeded, but Qwen's own confidence in the
        fix is below MIN_SOLUTION_CONFIDENCE. Rather than open a probably-
        wrong PR, post the analysis as a comment for a human to review --
        no branch, no fork, no PR. This is deliberately a soft stop, not
        a failure: the issue was understood, just not solved confidently
        enough to act on autonomously."""
        self._set_status(issue, IssueStatus.LOW_CONFIDENCE)
        issue.processed_at = datetime.now(timezone.utc)
        confidence = solution.get("confidence", 0)
        self._log(
            issue, "low_confidence_skip",
            f"Skipped opening a PR — confidence {confidence:.0%} is below the "
            f"threshold. Posted analysis as a comment instead.",
        )
        comment = (
            f"🤖 **DevInbox Analysis — Needs Human Judgment**\n\n"
            f"I investigated this issue and have a hypothesis, but my confidence "
            f"({confidence:.0%}) is below the threshold for opening a PR automatically.\n\n"
            f"**Analysis:** {solution.get('analysis', 'N/A')}\n\n"
            f"**Possible approach:** {solution.get('solution_approach', 'N/A')}\n\n"
            f"**Why I'm not confident:** {solution.get('potential_risks', 'The fix could not be fully verified against the codebase.')}\n\n"
            f"A maintainer's judgment is needed here rather than an automated guess.\n\n"
            f"---\n*Automated response by DevInbox AI Agent*"
        )
        self.github.post_comment(repo_full_name, issue_number, comment)
        self.db.commit()
    ...
    def _get_repository_context(self, repo_full_name: str) -> Optional[str]:
        try:
            ...
            lines.append(f"Files ({len(files)}{' (truncated)' if structure.get('truncated') else ''}):")
            for f in files[:250]:
